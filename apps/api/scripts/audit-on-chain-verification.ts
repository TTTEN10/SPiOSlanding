#!/usr/bin/env npx tsx
/**
 * On-Chain Audit Verification Script
 * 
 * Reconstructs ground truth from Sepolia chain:
 * 1. Fetches on-chain bytecode for each deployed contract
 * 2. Verifies proxy → implementation pointers
 * 3. Verifies contract wiring (Registry ↔ Ownership references)
 * 4. Checks role holders (DEFAULT_ADMIN_ROLE, MINTER_ROLE)
 * 
 * Run: npx tsx scripts/audit-on-chain-verification.ts
...
async function main() {
  ...
  // 3. Contract wiring (Registry ownershipContract, Ownership registryContract)
  console.log('\n3. CONTRACT WIRING');
  console.log('─────────────────');
  ...
  console.log('\n4. ROLE HOLDERS');
  console.log('───────────────');
  const timelockAddr = SEPOLIA_DEPLOYMENTS.GovernanceTimelock.toLowerCase();
